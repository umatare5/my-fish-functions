function grevdelete -a serviceName -a revisionName -a regionName --description 'alias grevdelete=gcloud run revisions delete serviceName revisionName --region regionName'
  ...
  echo "Running 'gcloud run revisions delete $SERVICE_NAME $REVISION_NAME --region $REGION_NAME' ..."
  gcloud run revisions delete $SERVICE_NAME $REVISION_NAME --region $REGION_NAME;
end
